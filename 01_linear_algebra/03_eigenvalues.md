...
意味: 「$A$ という変換を $\mathbf{v}$ に適用すると、**$\mathbf{v}$ が $\lambda$ 倍に伸びるだけで、向きは変わらない**」
...
行列 $A = \begin{pmatrix} 2 & 1 \\ 0 & 3 \end{pmatrix}$ の固有値・固有ベクトルは?

### 数式で
特性方程式 $\det(A - \lambda I) = 0$ を解く:

$$
\det\begin{pmatrix} 2-\lambda & 1 \\ 0 & 3-\lambda \end{pmatrix}
...
各 $\lambda$ について $(A - \lambda I)\mathbf{v} = 0$ を解いて固有ベクトルを得る:

$$
\lambda = 2: \mathbf{v} = (1, 0)^\top \quad \text{(x軸方向)}
$$

$$
\lambda = 3: \mathbf{v} = (1, 1)^\top \text{ を正規化したもの}
$$
...
$$
\text{データ} \to \text{共分散行列 } \Sigma \to \text{固有値分解} \to \text{大きい固有値の固有ベクトル}
$$
...
これを行列方程式で表すと、重要度ベクトル $\mathbf{r}$ は:

$$
\mathbf{r} = M \mathbf{r} \quad \text{(} M \text{ はリンク確率行列)}
$$
...
特に **対称行列 ($A = A^\top$)** は、固有値・固有ベクトルが**美しい性質**を持ちます:
...
NumPy の `eig` が返す固有ベクトルは、**ノルム $1$ に正規化**されています。
ただし**符号は不定** ($\mathbf{v}$ も $-\mathbf{v}$ も固有ベクトル)。
...
| 概念 | 数式 | Python | 意味 |
|---|---|---|---|
| 固有値・固有ベクトル | $A\mathbf{v} = \lambda \mathbf{v}$ | `np.linalg.eig(A)` | 向きが変わらない方向と倍率 |
| 対称行列の固有値 | – | `np.linalg.eigh(A)` | 実数・直交、安定計算 |
| 対角化 | $A = P D P^{-1}$ | `P @ diag(λ) @ inv(P)` | べき乗が一発 |
| スペクトル分解 | $A = Q \Lambda Q^\top$ | (eigh で取得) | 対称行列の美しい構造 |
...
- **量子力学のシュレーディンガー方程式** — $H\psi = E\psi$ という固有値問題
...
| ← 前 | 🏠 章 TOP | 📚 全体 TOP | 次 → |
|---|---|---|---|
| [`02_matrices.md`](02_matrices.md) | [章 TOP](README.md) | [📚 ROOT README](../README.md) | [`04_decompositions.md`](04_decompositions.md) |
