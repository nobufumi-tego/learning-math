...
> ✒️ **$\hat{f}$ の帽子 (ハット) の意味**: これは「$f$ をデータから**推定した**もの」という印です。
> 真の $f$ は神様しか知らず、私たちは $\hat{f}$ しか手にできない — だから
> 「$\hat{f}$ は $f$ にどれくらい近いか」を問うのが統計学であり、その答えが上の分解式です。
> ハット・バーなど記号の飾りの読み方 → [`00_notation/07_decorations_and_fonts.md`](../00_notation/07_decorations_and_fonts.md)
