...
と書きます。読み方: 「**n が無限大に近づくとき、(1/2)ⁿ の極限は 0**」。
...
`f(x) = sin(x) / x` という関数。
`x = 0` を入れると **0/0** になるので、本当は計算できないはず。

でも、**`x` をどんどん 0 に近づけていく** と…

```
x = 1.0     →  sin(1.0) / 1.0   = 0.8415
...
| 記号 | 読み方 | 意味 |
|---|---|---|
| `lim_{x→a} f(x)` | "the limit of f(x) as x approaches a" / 「x が a に近づくときの f(x) の極限」 | x が a に限りなく近づくときの値 |
| `lim_{x→∞}` | "as x approaches infinity" | x が無限大に近づくとき |
| `lim_{x→0⁺}` | "approaches 0 from the right" | 右から 0 に近づく (正の側) |
| `lim_{x→0⁻}` | "approaches 0 from the left" | 左から 0 に近づく (負の側) |

> 💡 **`x → a` は「x = a」ではない**。「**a に近づくが、a そのものは含まない**」がポイント。
...
出力:
```
x = 1.000000  →  sin(x)/x = 0.8414709848
x = 0.100000  →  sin(x)/x = 0.9983341665
x = 0.010000  →  sin(x)/x = 0.9999833334
x = 0.001000  →  sin(x)/x = 0.9999998333
x = 0.000100  →  sin(x)/x = 0.9999999983
x = 0.000010  →  sin(x)/x = 1.0000000000
```
...
# lim x→0 (1+x)^(1/x)  ← e の定義そのもの!
print(sp.limit((1 + x)**(1/x), x, 0))    # E (= e ≈ 2.718...)

# lim x→∞ (1 + 1/x)^x  ← e のもう1つの定義
...
`x = 1` の前後で:
- `x → 1⁻` (左から近づく): `f(0.999) = 0`
- `x → 1⁺` (右から近づく): `f(1.001) = 1`
...
ソフトマックスでよく見る `Z = Σ exp(zᵢ)` も、極限的に「どの z が支配的か」を見るときに、**温度パラメータ τ** を変化させて極限を考えます:

- `τ → ∞`: すべて一様 (どれも同じ確率)
- `τ → 0`: 最大値だけ確率 1 (argmax と同じ)
...
- **「近づく」≠「等しい」**: `lim_{x→a} f(x) = L` でも `f(a) ≠ L` のことがある (関数の値が定義されてない場合など)
- **左右の極限が違うことがある**: 階段関数のように、左から近づくか右から近づくかで値が変わる
- **数値計算は精度に注意**: あまりに小さい h を使うと、浮動小数点誤差で逆に精度が落ちる (例: `h = 1e-30` だと壊れる)
...
| 概念 | 数式 | Python | 意味 |
|---|---|---|---|
| 極限 | `lim_{x→a} f(x)` | `sympy.limit(f, x, a)` | 限りなく近づく値 |
| 無限大 | `∞` | `numpy.inf` / `sympy.oo` | 無限 |
| 連続 | `lim f(x) = f(a)` | グラフがジャンプしない | – |
| 自然対数 e の定義 | `lim (1+1/n)^n` | `(1+1/n)**n` | 連続複利の限界 |
...
- **e (ネイピア数)** — `lim (1+1/n)^n` で定義される自然対数の底
- **L'Hôpital の定理** — `0/0` 形の極限を解く必殺技
