...
f(x, y) = x² + y²        ← 2変数
f(x, y, z) = x + 2y + 3z ← 3変数
f(x₁, x₂, ..., xₙ)        ← n変数 (ML では n = 数億)
```

機械学習では、入力データのベクトル `x ∈ ℝⁿ` や、モデルパラメータ `w ∈ ℝᵈ` を引数に取るので、**ほぼ全ての関数が多変数関数**。
...
`∂` (ラウンド・ディー) は普通の `d` と区別するための記号。**「他の変数は固定して、x だけ動かす」** という意味。
...
`f(x, y)` を「ある点の標高」と思うと:
- `∂f/∂x`: **東 (x方向) に1歩進んだときの標高変化**
- `∂f/∂y`: **北 (y方向) に1歩進んだときの標高変化**
...
`z = f(g(x), h(x))` のような、内部で多変数を経由するケース:
...
> **`∇f = 0`** (すべての偏微分がゼロ) になる点が **極値の候補**

例えば `f(x, y) = (x-1)² + (y-2)²` の最小値は `(1, 2)` で、そこで `∂f/∂x = ∂f/∂y = 0`。
...
多変数関数は `(x, y) → z` の 3 次元グラフですが、紙に描くとき **等高線** で表現するのが便利:
...
損失関数 `L(w₁, w₂, ..., w_{1.75兆})` は **1750 億変数の関数**。

各パラメータ `wᵢ` についての偏微分 `∂L/∂wᵢ` を**全部一気に計算**する必要があります。
...
- **どの変数で微分するかを明示**: `∂f/∂x` vs `∂f/∂y` 混同しない
...
| 概念 | 数式 | Python |
|---|---|---|
| 偏微分 | `∂f/∂x` | `sympy.diff(f, x)` / `jax.grad` |
| 多変数関数 | `f(x, y, z, ...)` | `lambda p: ...` |
| 勾配ベクトル | `∇f = (∂f/∂x₁, ..., ∂f/∂xₙ)` | `jax.grad(f)(p)` |
| 極値の必要条件 | `∇f = 0` | `grad_f(p) ≈ 0` |
