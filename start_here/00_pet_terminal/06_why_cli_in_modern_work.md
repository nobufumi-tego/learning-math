...
(`magick` は ImageMagick という画像加工ツール)
...
**3秒**で答えが出ます。
(`-F','` でカンマ区切り、`$3` で3列目、`sum+=` で合計、`END {print sum}` で最後に表示)
...
**1秒**。
これを **「毎朝6時に自動実行」** する設定 (cron) を仕込めば、もう手動で見に行く必要すらない。
