...
(すでに [`docs/setup.md`](../../docs/setup.md) にも書いてありますが、ここにも置いておきます)
...
```
       ／￣￣＼
      |  ・ω・  |   ぼくは、ブラウザを閉じても止まらないよ。
      |   ⌒    |   起動したターミナルで Ctrl+C を 2回 押してね。
       ＼____／
```
...
|---|---|
| 起動 | `uv run lab.py` |
| **停止** | 起動したターミナルで **`Ctrl+C` を 2回** |
| 停止 (UI から) | JupyterLab 画面の上部 **File → Shut Down** |

> ⚠️ **超重要**: ブラウザのタブを `×` で閉じても、**Jupyter サーバーはバックグラウンドで動き続けます**。
>
> 止め忘れて翌日また `uv run lab.py` をすると、**「Port 8888 is already in use」** というエラーが出ます。
> このエラーが出たら、ペンタが「**まだ起きてるよ!**」と言ってるサインです。
>
> 救済方法は [`docs/setup.md`](../../docs/setup.md) の「ステップ 3.5」を参照。

#### 🗣️ 英語ダイアログが出てもドキッとしない

JupyterLab で作業中、**急に英語のダイアログ** がポップアップして焦ること、絶対にあります。

```
       ／￣￣＼
      |  ・ω・  |   "Save your work?" って出てきた!?
      |   ⌒    |   ふつうは "Save" を押せば OK だよ。
       ＼____／
```
...
| 英語 | 意味 | 通常の選択 |
|---|---|---|
| **Save your work** | 変更を保存する? | **Save** |
| **Shut down JupyterLab** | JupyterLab を終了する? | **Shut Down** (終わるとき) |
| **Restart kernel** | カーネル再起動? (変数全消去) | **Cancel** で考え直す |

全 6 種類の翻訳と「**どのボタンを押すべきか**」は [`docs/setup.md` の「ステップ 3.6」](../../docs/setup.md#ステップ-36-jupyterlab-でよく見る英語ダイアログ-) を参照。

> 💡 **小ワザ**: 編集中こまめに **`Ctrl+S`** (Mac は `⌘S`) を押す習慣を付けると、「Save your work?」 ダイアログがほぼ出なくなります。
