...
> ### ✒️ この記号の生まれ — `∈` と `∅`
>
> `∈` は **1889 年**、イタリアの数学者 **ペアノ**が導入しました。
> 由来はギリシャ語 **ἐστί (esti)**、英語の *is* にあたる語の頭文字 **ε** です。
> つまり $x \in A$ は「$x$ は $A$ で**ある**」という**一つの文**として読めます。
>
> `∅` のほうは、実は**ギリシャ文字 φ ではありません**。
> ノルウェー語・デンマーク語のアルファベット **Ø** です。
> 導入したのは「ニコラ・ブルバキ」— **実在しない架空の数学者**（フランスの数学者集団のペンネーム）でした。
>
> 📖 続き → [`columns/03_logic_symbols_are_new.md`](columns/03_logic_symbols_are_new.md)
