...
**この段階のゴール**: 真っ黒な画面に拒否反応がなくなり、`uv run jupyter lab` が打てる。
...
- 「rm はゴミ箱に入らない」と知っている
- `uv sync` と `uv run jupyter lab` が何をしているか説明できる
...
- Phase −1: 真っ黒な画面が怖くない。`uv run jupyter lab` を打って、Jupyter を立ち上げられる
