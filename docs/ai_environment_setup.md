# AI 学習環境のセットアップ — clone / ZIP 展開 直後から本環境と同等の体験を

このリポジトリは、Claude Code / Gemini CLI / Codex CLI を**プロジェクト固有の設計知 (memory) 込みで配布**しています。
**git clone でも、ZIP ダウンロード&展開でも**、AI が「**この repo の流儀**」を理解した状態で動き出します。

> 💡 取得方法は `git clone` または GitHub の「Download ZIP」 のどちらでもOK。詳細は [`docs/setup.md`](setup.md) のステップ 1 参照。
...
取得後 (clone / ZIP どちらでも)、以下が動けば AI 環境は本環境と同等:
